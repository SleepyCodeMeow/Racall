...
The Windows NSIS installer includes English and Russian with an explicit language selector. `installer/languages.nsh` writes the selected locale to `<install directory>/resources/installer-language.json`. It does not write to the installing administrator's personal profile.
...
4. If offering it during Windows installation, add the NSIS locale and mapping to `installer/languages.nsh` and `package.json`.
